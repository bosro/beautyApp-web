...
function startRegistration(options) {
  return __async(this, null, function* () {
    ...
    const publicKey = __spreadProps(__spreadValues({}, optionsJSON), {
      challenge: base64URLStringToBuffer(optionsJSON.challenge),
      user: __spreadProps(__spreadValues({}, optionsJSON.user), {
    ...
    let credential;
    try {
      credential = yield navigator.credentials.create(createOptions);
    } catch (err) {
...
function startAuthentication(options) {
  return __async(this, null, function* () {
    ...
    let credential;
    try {
      credential = yield navigator.credentials.get(getOptions);
    } catch (err) {
