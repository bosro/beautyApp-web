  ...
  template: `
    <div class="min-h-screen bg-[var(--color-background)] pb-24 lg:pb-8">
      ...
      <div class="sticky top-0 z-10 bg-[var(--color-surface)] border-b border-[var(--color-border)] px-4 py-4 flex items-center justify-between">
        ...
        <button (click)="router.navigate(['/beautician/business-profile'])" class="text-sm text-[var(--color-primary)] font-medium flex items-center gap-1">
      ...
      <div *ngIf="!loading" class="p-4 lg:p-6 max-w-2xl mx-auto space-y-4">
        ...
        <div class="card p-5 flex flex-col items-center gap-3">
          <div class="relative">
            <img
              [src]="previewUrl || user?.avatar || 'https://ui-avatars.com/api/?name=' + user?.firstName + '&size=96&background=E88B7B&color=fff'"
              alt="Profile"
              class="w-24 h-24 rounded-full object-cover ring-4 ring-[var(--color-primary)]"
            />
            <label class="absolute bottom-0 right-0 w-8 h-8 bg-[var(--color-primary)] rounded-full flex items-center justify-center cursor-pointer hover:opacity-90">
              <i class="ri-camera-line text-white text-sm"></i>
              <input type="file" accept="image/*" class="hidden" (change)="onFile($event)" />
            </label>
          </div>
          <div class="text-center">
            <h2 class="font-bold text-lg text-[var(--color-text-primary)]">{{ user?.firstName }} {{ user?.lastName }}</h2>
            <p class="text-sm text-[var(--color-text-secondary)]">{{ user?.businessName }}</p>
            <div class="flex items-center justify-center gap-1 mt-1">
              <i class="ri-star-fill text-amber-400 text-sm"></i>
              <span class="text-sm font-semibold">{{ user?.averageRating | number:'1.1-1' }}</span>
              <span class="text-xs text-[var(--color-text-muted)]">({{ user?.reviewCount }} reviews)</span>
            </div>
          </div>
          <button *ngIf="selectedFile" (click)="uploadPhoto()" [disabled]="uploading" class="btn-primary text-sm px-4 py-2">
        ...
        <div class="card p-4 space-y-4">
          ...
          <div>
            <label class="block text-sm font-medium text-[var(--color-text-secondary)] mb-1.5">Phone</label>
          ...
          <div>
            ...
            <input [value]="user?.email" type="email" class="form-input bg-[var(--color-background)]" readonly />
          </div>
          <button (click)="savePersonal()" [disabled]="saving" class="btn-primary w-full">
        ...
        <div class="grid grid-cols-2 gap-3">
          <div class="card p-4 text-center">
            <p class="text-2xl font-black text-[var(--color-primary)]">{{ stats?.totalCompletedBookings || 0 }}</p>
            <p class="text-xs text-[var(--color-text-muted)]">Completed</p>
          </div>
          <div class="card p-4 text-center">
            <p class="text-2xl font-black text-[var(--color-primary)]">{{ stats?.totalClients || 0 }}</p>
...
export class BeauticianProfileComponent implements OnInit {
  ...
  uploadPhoto() {
    ...
    this.http.post<any>(`${environment.apiUrl}/users/avatar`, fd).subscribe({
      next: (res) => { this.auth.updateUser(res.data); this.uploading = false; this.selectedFile = null; this.toast.success('Photo updated!'); },
  ...
  savePersonal() {
    this.saving = true;
    this.http.put<any>(`${environment.apiUrl}/users/profile`, { firstName: this.firstName, lastName: this.lastName, phone: this.phone }).subscribe({
      next: (res) => { this.auth.updateUser(res.data); this.saving = false; this.toast.success('Profile updated!'); },
