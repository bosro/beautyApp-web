        ...
        >
          ...
          <div class="flex-1 min-w-0">
            <p
              ...
              [ngClass]="{ 'font-semibold': !notif.isRead }"
            >
              {{ notif.title }}
            </p>
            <p
              class="text-sm text-[var(--color-text-secondary)] mt-0.5 line-clamp-2"
            >
              {{ notif.message }}
            </p>
          ...
          <div
            *ngIf="!notif.isRead"
            class="w-2 h-2 rounded-full bg-[var(--color-primary)] flex-shrink-0 mt-1.5"
          ></div>
        </div>
      </div>
    </div>
...
export class NotificationsComponent implements OnInit {
  ...
  ngOnInit() {
    this.http.get<any>(`${environment.apiUrl}/notifications`).subscribe({
      next: (res) => {
        this.notifications = res.data?.notifications || []; // was: res.data || []
        this.loading = false;
      },
  ...
  markRead(notif: any) {
    ...
    this.http
      .put(`${environment.apiUrl}/notifications/${notif.id}/read`, {})
      .subscribe();
    notif.isRead = true;
  }
