        ...
        >
          ...
          <div class="p-3">
            <div class="flex items-start justify-between gap-2">
              ...
              <div
                class="flex items-center gap-1 flex-shrink-0 bg-amber-50 dark:bg-amber-900/20 px-1.5 py-0.5 rounded-lg"
              >
                <i class="ri-star-fill text-amber-400 text-xs"></i>
                <span
                  class="text-xs font-semibold text-amber-600 dark:text-amber-400"
                  >{{ fav.beautician.averageRating | number: "1.1-1" }}</span
                >
              </div>
            </div>
            <div class="flex items-center gap-2 mt-2">
              <span
                *ngIf="fav.beautician.category"
                class="badge badge-info text-xs"
                >{{ fav.beautician.category }}</span
              >
              <span class="text-xs text-[var(--color-text-muted)]"
                >{{ fav.beautician.reviewCount }} reviews</span
              >
            </div>
          </div>
        </div>
...
export class FavoritesComponent implements OnInit {
  ...
  loadFavorites() {
    ...
    this.http.get<any>(`${environment.apiUrl}/favorites`).subscribe({
      next: (res) => {
        this.favorites = res.data?.favorites || []; // was: res.data || []
        this.loading = false;
      },
