...
@Component({
  ...
  template: `
    <div class="min-h-screen bg-[var(--color-background)] pb-24 lg:pb-8">
      <!-- Header with search -->
      <div
        class="sticky top-0 z-10 bg-[var(--color-surface)] border-b border-[var(--color-border)] px-4 py-3 space-y-3"
      >
        <div class="flex items-center gap-3">
          ...
          >
            <i
              class="ri-map-pin-2-line text-xl text-[var(--color-primary)]"
            ></i>
          </button>
          <div class="flex-1 relative">
            <i
              class="ri-search-line absolute left-3 top-1/2 -translate-y-1/2 text-[var(--color-text-muted)]"
            ></i>
            <input
              [(ngModel)]="query"
              (keyup.enter)="search()"
          ...
          <button (click)="search()" class="btn-primary px-4 py-2.5 text-sm">
            Search
          </button>
        </div>
      ...
      <div *ngIf="!loading && !searched" class="p-4 text-center mt-12">
        ...
        <p class="text-[var(--color-text-secondary)] mt-2">
          Search for salons or beauty services
        </p>
      </div>
      ...
      <div
        *ngIf="!loading && results.length > 0"
        class="p-4 lg:p-6 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4"
      >
        <div
        ...
        >
          ...
          <div class="p-3">
            ...
            <p
              class="text-xs text-[var(--color-text-secondary)] mt-0.5 flex items-center gap-1"
            >
              <i class="ri-map-pin-line"></i> {{ item.city || "Accra" }}
            </p>
            <div class="flex items-center justify-between mt-2">
              <span *ngIf="item.category" class="badge badge-info text-xs">{{
                item.category
              }}</span>
