      ...
      <form [formGroup]="form" (ngSubmit)="onSubmit()" class="space-y-4">
        <div>
          ...
          <p *ngIf="!(submitted && f['name'].errors)" class="text-xs mt-1" style="color: var(--color-text-secondary)">
            Use your salon or brand name — this is what customers will see
          </p>
        </div>
        ...
        <div>
          ...
          <p *ngIf="submitted && f['phone'].errors" class="text-xs text-red-500 mt-1">Phone number is required</p>
          <p *ngIf="!(submitted && f['phone'].errors)" class="text-xs mt-1" style="color: var(--color-text-secondary)">
            Format: +233 50 123 4567 — clients use this to reach you
          </p>
        </div>
        ...
        <div>
          ...
          <p *ngIf="submitted && !f['confirmPassword'].errors && form.errors?.['mismatch']" class="text-xs text-red-500 mt-1">Passwords don't match</p>
...
export class BeauticianRegisterComponent implements OnInit {
  ...
  onGoogleSignIn(): void {
    ...
    // for the full reasoning. Note: the categorization details collected
    // in the modal above can't survive this particular round trip
    // (Google's redirect only carries `state` back to our backend, not
    // arbitrary form data) — they can still fill these in afterwards from
    // their profile.
